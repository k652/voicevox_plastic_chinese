...
def generate_app(
    ...
    @app.post(
        "/read",
        # response_class=AudioQuery,
        tags=["音声合成"],
        summary="oneclick音声合成する",
    )
    def read(text: str, speaker: int, speed: float, core_version: Optional[str] = None):
        """
        クエリの初期値を得ます。ここで得られたクエリはそのまま音声合成に利用できます。各値の意味は`Schemas`を参照してください。
        """
        engine = get_engine(core_version)
        accent_phrases = engine.create_accent_phrases(text, speaker_id=speaker)
        txt_query =  AudioQuery(
            accent_phrases=accent_phrases,
            speedScale=speed,
            pitchScale=0,
            intonationScale=1,
        ...
        # with NamedTemporaryFile(dir=r"D:\VOICEVOX\audio_tmp", delete=False) as f:
        with open(f"D:\\VOICEVOX\\audio_tmp\\{time.time()}{text[:10].strip()}-{speaker}.wav", 'wb') as f:
            soundfile.write(
                file=f, data=wave, samplerate=txt_query.outputSamplingRate, format="WAV"
            )
        try:
